...
class _Tela10State extends State<Tela10> {
  ...
  final Map<String, IconData> iconMappings = {
    ...
    'pets': Icons.pets, // Alteração para Pets
  ...
  void _filterProducts(String searchTerm) {
    setState(() {
      ...
      if (searchTerm.isEmpty) {
        // Caso a barra de pesquisa esteja vazia, ative o filtro 'Pets'
        filteredProducts = allProducts.where((product) => product.tags.contains('Pets')).toList();
      } else {
        // Caso contrário, aplique os filtros normais
        filteredProducts = allProducts.where((product) {
          return (ignoreInitialFilter || product.tags.contains('Pets') || searchTerm.isNotEmpty) &&
              (searchTerm.isEmpty ||
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Pets'), // Alteração para Pets
        actions: [
          IconButton(
